...
export default async function getPageProperties(
  ...
  // Debug: log raw properties before processing
  console.log(`[getPageProperties] id=${id}, raw: Published=${properties.Published}, type=${properties.type}, status=${properties.status}`)

  // 处理 Published checkbox：将 "Yes"/"No" 映射为 status
  if (properties.Published && !properties.status) {
    properties.status = properties.Published === 'Yes' ? ['Published'] : ['Invisible']
  }
  ...
  // Debug: log final property values
  console.log(`[getPageProperties] id=${id}, final: type=${properties.type}, status=${properties.status}, slug=${properties.slug}`)
